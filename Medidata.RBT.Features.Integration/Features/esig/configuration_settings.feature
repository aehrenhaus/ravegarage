...
#Some of these configuration settings will not apply to externally authenticated users. Rave rules concerning authentication will not apply to iMedidata or IDP users. iMedidata will set its own authentication rules, verify user creadentials prior to allowing the user to enter Rave and will provide authorization criteria to Rave so that the user permissions are understood by Rave. IDPs will act in a manner similar to iMedidata in that they will control authentication and provide authorization criteria to Rave as well.
#Aditionally, both iMedidata and IDP will provide eSignature authorization for users that utilize these methods to authenticate to Rave. Since the eSignature request is being handled outside of Rave, the authentication settings will be handled by the authenticator. Rave eSignature configuration options will not apply to external users.
