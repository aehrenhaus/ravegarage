...
Scenario: If an iMedidata user has a  new study assignment added in iMedidata, that study assignment appears in Rave.
    ...
    And I click on iMedidata Link to navigate back to iMedidata
    And I am on iMedidata Home page
	And have a invitation to "<Study C>" for App "<EDC App>" role "<EDC role 1>  App "<Modules App>" with Role "<Modules Role 1>" in iMedidata
...
@PB2.5.1.24-02
@Validation
Scenario: If the user is an iMedidata user, when the user that has only one EDC Role subsequently accesses a specific Study in Rave from iMedidata, and that user has at least one site assignment for that study in Rave, then Rave will direct the user to that Site page on Rave.
	...
	And I have been invited to the Site "<Site A1>" for Study "<Study A>"
	And I accept the invitation to the Site "<Site A1>"
	When I follow app "<EDC App>" for Study "<Study A>"
	Then I should be on Site "<Site A1>" page in Rave for Study "<Study A>"
	And I take a Screenshot

@Rave 564 Patch 13
@PB2.5.1.24-03
@Validation
Scenario: If the user is an iMedidata user, when the user that has only one EDC Role subsequently accesses the Rave EDC App for a Study in iMedidata, then Rave will direct the user to the Rave Study Homepage.
...
Scenario: If the user is an iMedidata user, when the user that has only one EDC Role and has an assignment to a Study Group with Multiple Studies in 
    ...
    And there is an iMedidata Study Group named "<Study Group>"
    And there is an iMedidata Study named "<Study A>" "<Study B>" "<Study C>" in the Study Group named "<Study Group>"
	And I have an assignment to the iMedidata Study Group named "<Study Group>"
    And I have an assignment to the iMedidata Study named "<Study A>" for the App named "<EDC App>" Role "<EDC Role 1>" 
	...
	When I follow "<EDC App>" for Study Group named "<Study Group>"
...
@Validation
@BUG
Scenario: If a user has assignments to the ‘Rave Modules’ application for different studies with different roles before accessing Rave, then the user will not be provided access to Rave , and the message will be displayed  when the user tries to access Rave from iMedidata “You have accepted "X" "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role.”
	...
	Then I should see an Error message "You have accepted 3 "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role."
	...
	And I should see an Error message "You have accepted 3 "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role."
	And I navigate back to iMedidata
...
@PB2.7.5.12-02
@Validation
Scenario: If a user has assignments to the ‘Rave Modules’ and "EDC" application for a study and EDC Application only to a another study, then the user will not be provided access to Rave , and the message will be displayed  when the user tries to access Rave from iMedidata “You have accepted "X" "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role.”	
    ...
    And I have an assignment to the iMedidata Study named "<Study B>" for the App named "<EDC App>"  Role "<EDC Role 1>"
	When I follow "<EDC App>" for Study "<Study A>" 
	Then I should see an Error message "You have accepted 2 "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role."
	And I take Screenshot
	And I should not be on Rave Study "<Study A>" homepage
	And I navigate back to iMedidata
	And I follow "<EDC App>" for Study "<Study B>" 
	And I should see an Error message "You have accepted 2 "Rave Modules" invitations in iMedidata. You can be assigned to only one role for Rave Modules. Please contact your study owner so you can be removed and invited again with one role."
	And I take Screenshot
