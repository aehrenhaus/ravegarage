...
Feature: US18531_DT13626 When objects are added and then removed by an edit check with a merge matrix check action, duplicate records should not show up in the database after migration.
...
@release_2012.1.0
@PB_US18531_DT13626_10
@Validation
Scenario: PB_US18531_DT13626_10 When the data is soft deleted and subject is migrated, then extra records should not be created in the database
...
And I navigate to Form "Form B"
And I record the datapageID
And I take a screenshot
And I navigate to form "Form A"
And I enter data in CRF and save
    | Field   | Data | Control Type |
...
And I take a screenshot
And I log in to the testing database
When I execute the following query "select * from datapages where datapageID = <previously recorded datapageID>"
And I execute the following query "select * from records where datapageID = <previously recorded datapageID>"
And I execute the following query "select * from datapoints where datapageID = <previously recorded datapageID>"
